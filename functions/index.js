 ...
 */
exports.updateLeaderboardOnSession = functions.database.ref('/users/{uid}/sessions/{sessionId}')
    .onCreate(async (snapshot, context) => {
        const session = snapshot.val();
        const uid = context.params.uid;

        // Calculate points securely on the server
        const pointsEarned = (session.cardsStudied || 0) * 10 + (session.correctCount || 0) * 5;
        ...
        return console.log(`Securely updated points for ${uid}: +${pointsEarned}`);
    });

/**
 * 2. SYNC PROFILE TO LEADERBOARD
 * Ensures leaderboard display names and avatars stay in sync with profile edits.
 */
exports.updateLeaderboardOnProfile = functions.database.ref('/users/{uid}/profile')
